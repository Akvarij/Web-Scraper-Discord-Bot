...
async function scrape(sportsChannel, options = {}) {
  ...
  try {
    ...
    await page.goto("https://protime.si/sl_si/dogodki/", {
      waitUntil: "load",
      timeout: 60000,
    ...
    if (events.length > 0) {
      const eventString = events
        .map(
          (event) =>
            `Date: ${event.Date}, Title: ${event.Title}, Link: <${event.Link}>`
        )
        .join("\n");
    ...
    if (scheduled) {
      await sportsChannel.send(
        "@here An error occurred while scraping the page."
      );
    } else if (msg) {
